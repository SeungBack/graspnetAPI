...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def eval_scene(self, scene_id, dump_folder, TOP_K = 50, return_list = False,vis = False, max_width = 0.1, mini=False, ann_ids=None):
        '''
        **Input:**
        ...
        **Output:**
        ...
        # Load per-scene shared files once (camera_poses has one entry per annotation)
        _scene_cam_path = os.path.join(self.root, 'scenes', get_scene_name(scene_id), self.camera)
        _camera_poses_all = np.load(os.path.join(_scene_cam_path, 'camera_poses.npy'))
        _align_mat = np.load(os.path.join(_scene_cam_path, 'cam0_wrt_table.npy'))
        ...
        for ann_id in ann_range:
            ...
            _ann_reader = xmlReader(os.path.join(_scene_cam_path, 'annotations', '%04d.xml' % ann_id))
            pose_list = [parse_posevector(pv)[1] for pv in _ann_reader.getposevectorlist()]
            camera_pose = _camera_poses_all[ann_id]
            align_mat = _align_mat
    ...
    def generate_mini_mixed_ann_ids(self, save_path, seed=42, min_ann=1, max_ann=50):
        '''
        Generate and save a fixed random mapping of scene_id -> ann_ids for test_mini_mixed.

        **Input:**

        - save_path: string path to save the JSON file.

        - seed: int random seed for reproducibility.

        - min_ann: int minimum number of annotations per scene.

        - max_ann: int maximum number of annotations per scene.

        **Output:**

        - scene_ann_ids: dict mapping scene_id (int) -> list of ann_ids.
        '''
        rng = np.random.default_rng(seed)
        scene_ids = list(range(100, 190))
        scene_ann_ids = {}
        for scene_id in scene_ids:
            n = int(rng.integers(min_ann, max_ann + 1))
            ann_ids = sorted(rng.choice(256, size=n, replace=False).tolist())
            scene_ann_ids[scene_id] = ann_ids
        with open(save_path, 'w') as f:
            json.dump(scene_ann_ids, f)
        print(f'Saved mini_mixed ann_ids to {save_path}')
        return scene_ann_ids

    def eval_mini_mixed(self, dump_folder, ann_ids_path, proc=2):
        '''
        Evaluate test splits using a fixed per-scene random subset of annotations loaded from JSON.

        **Input:**

        - dump_folder: string of the folder that saves the npy files.

        - ann_ids_path: string path to the JSON file generated by generate_mini_mixed_ann_ids.
        ...
        **Output:**
        ...
        with open(ann_ids_path, 'r') as f:
            scene_ann_ids_str = json.load(f)
        scene_ann_ids = {int(k): v for k, v in scene_ann_ids_str.items()}
        ...
        for k, v in info.items():
            ...
            res_list = []
            for scene_id in v:
                res_list.append(p.apply_async(self.eval_scene, (scene_id, dump_folder), {'ann_ids': scene_ann_ids[scene_id]}))
            p.close()
            p.join()
            # Each scene may have a different number of annotations, so concatenate instead of stacking
            all_accs = np.concatenate([np.array(r.get()) for r in res_list], axis=0)  # (total_anns, TOP_K, 6)
            ap = np.mean(all_accs)
            res_mean = np.mean(all_accs.reshape(-1, 6), axis=0)
            print('\n' + '='*50)
            print(f"Split: {k} Mini-Mixed")
            print('='*50)
            print(f"AP@0.4: {res_mean[1]*100:.2f}%")
            print(f"AP@0.8: {res_mean[3]*100:.2f}%")
            print(f"AP    : {np.mean(res_mean)*100:.2f}%")
            print('='*50 + '\n')
            results_dict[k] = {'res': res_mean, 'ap': ap}
        return results_dict
    ...
    def eval_all(self, dump_folder, proc = 2):
        ...
        **Output:**
        ...
        # print AP@0.4, AP@0.8, AP for test_seen, test_similar, test_novel
        res_seen = res[0:30]
        res_similar = res[30:60]
        res_novel = res[60:90]
        
        # print AP@0.4, AP@0.8, AP for test_seen, test_similar, test_novel
        for split_name, split_res in zip(['Seen', 'Similar', 'Novel'], [res_seen, res_similar, res_novel]):
            split_res_reshaped = np.array(split_res).reshape(-1, 6)
            split_res_mean = np.mean(split_res_reshaped, axis=0)
            print('\n' + '='*50)
            print(f"Split: {split_name}")
            print('='*50)
            print(f"AP@0.4: {split_res_mean[1]*100:.2f}%")
            print(f"AP@0.8: {split_res_mean[3]*100:.2f}%")
            print(f"AP    : {np.mean(split_res_mean)*100:.2f}%")
            print('='*50 + '\n')
        ap = np.mean(res)
        res = np.array(res).reshape(-1, 6)
        res = np.mean(res, axis=0)
